...
def song_requests(sock, message, url):
    ...
    if song == None:
        ...
        return chat(sock, "{}, be sure your link is for open.spotify.com In the next 5 minutes, if you use !requeue with a new link you can try again.".format(name), cfg.CHAN)
    elif "track" not in song:
        screwups[name] = (True, time.time())
        return chat(sock, "{}, you have to link to a Spotify song for song requests. In the next 5 minutes, if you use !requeue with a new link you can try again.".format(name), cfg.CHAN)
    else:
        songl = [song]
        try:
            ...
            screwups[name] = (True, time.time())
            return chat(sock, "{}, that returned an error. In the next 5 minutes, if you use !requeue with a new link you can try again.".format(name), cfg.CHAN)
        track = track1['id']
        ...
        if length > 360000:
            screwups[name] = (True, time.time())
            return chat(sock, "{}, we have a 6 minute limit on songs. In the next 5 minutes, if you use !requeue with a new song you can try again.".format(name), cfg.CHAN)
        elif len(tracks) < 1:
            ...
            if duplicate:
                screwups[name] = (True, time.time())
                return chat(sock, "{}, sorry that song is already in queue! In the next 5 minutes, if you use !requeue with a new song you can try again.".format(name), cfg.CHAN)
            else:
                sp.user_playlist_add_tracks(user, playlist, songl, position=None)
                screwups[name] = (False, time.time())
                return chat(sock, "{} song added to playlist".format(name), cfg.CHAN)
...
def clear_playlist(sock, message):
    ...
    if message['mod'] == '1' or 'broadcaster' in message['badges']:
        ...
        response = sp.user_playlist_remove_all_occurrences_of_tracks(user, playlist,
                                                          track_list)
...
def requeue(sock, message):
    ...
    if name in screwups.keys():
        if screwups[name][0] and time.time() - screwups[name][1] <= 300:
            ...
            del screwups[name]
            return chat(sock, "{}, sorry it's been more than 5 minutes".format(name), cfg.CHAN)
